...
function showdata()
{
 $.ajax({
	type: "GET",
	url: "/index.php?m=wap&c=common123&a=indexajax&category=blog&p="+(page+1),
	dataType: "text",
...
?>
  <td ><a class="show0<?php echo $i;?>" title="<?php echo $row->name;?>" href="<?php echo $site_url;?>wap/common123/index/category_<?php echo $row->id;?>"><?php echo $row->name;?></a></td>
  <?php echo ($i % 4==0) ? "</tr>
  <tr>" : "";?>
<?php $i++; }?>
...
?> 
<a title="<?php echo $row->title;?>" href="<?php echo $site_url;?>wap/common123/show/<?php echo $row->id;?>">
    <h3><img src="<?php echo (@$row->more) ? ((substr(@$row->more,0,4)=='http' || substr(@$row->more,0,1)=='/') ? @$row->more : $base_url.@$row->more)  : "";?>"/></h3>
