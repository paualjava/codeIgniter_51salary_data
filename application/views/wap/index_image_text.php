...
function showdata()
{
 $.ajax({
	type: "GET",
	url: "/index.php?m=wap&c=index_image_text&a=indexajax&category=blog&p="+(page+1),
	dataType: "text",
...
?>
  <td ><a class="show0<?php echo $i;?>" title="<?php echo $row->name;?>" href="<?php echo $site_url;?>wap/index_image_text/index/category_<?php echo $row->id;?>"><?php echo $row->name;?></a></td>
  <?php echo ($i % 4==0) ? "</tr>
  <tr>" : "";?>
<?php $i++; }?>
...
?> 
<a title="<?php echo $row->title;?>" href="<?php echo $site_url;?>wap/index_image_text/show/<?php echo $row->id;?>">
    <h3><img src="<?php echo (@$row->pic) ? ((substr(@$row->pic,0,4)=='http' || substr(@$row->pic,0,1)=='/') ? @$row->pic : $base_url.@$row->pic)  : "";?>"/></h3>
    <h4><?php echo $row->title;?></h4>
    <p><?php echo msubstr(strip_tags($row->content),0,80);?></p>
    <div class="clear"></div>
  </a>
 <?php $i++;}?> 
