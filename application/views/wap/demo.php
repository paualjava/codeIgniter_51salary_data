...
?>
<td>
      <a class="a<?php echo $i;?> route0<?php echo $i;?>" title="<?php echo $row->name;?>" href="<?php echo $site_url;?>wap/demo/index/category_<?php echo $row->id;?>"><span><?php echo msubstr($row->name,0,3);?></span></a>
    </td>
<?php echo ($i % 4==0) ? "</tr>
  <tr>" : "";?>
<?php $i++; }?>
...
function formCheck()
{
     ...
     <form name="search" type="get" action="<?php echo $site_url;?>wap/demo" id="search" onsubmit="return formCheck();">
...
?>  
<tr><td><a  title="<?php echo $row->title;?>" href="<?php echo $site_url;?>wap/demo/show/<?php echo $row->id;?>"><img src="<?php echo (@$row->pic) ? ((substr(@$row->pic,0,4)=='http' || substr(@$row->pic,0,1)=='/') ? @$row->pic : $base_url.@$row->pic)  : "";?>" /><span><?php echo $row->title;?></span></a></td></tr>
      <?php $i++;}?>
