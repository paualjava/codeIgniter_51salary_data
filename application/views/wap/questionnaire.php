...
function showdata()
{
 $.ajax({
	type: "GET",
	url: "/index.php?m=wap&c=questionnaire&a=indexajax&category=blog&p="+(page+1),
	dataType: "text",
...
?>
  <td ><a class="show0<?php echo $i;?>" title="<?php echo $row->name;?>" href="<?php echo $site_url;?>wap/questionnaire/index/category_<?php echo $row->id;?>"><?php echo $row->name;?></a></td>
  <?php echo ($i % 4==0) ? "</tr>
  <tr>" : "";?>
<?php $i++; }?>
...
?> 
<a title="<?php echo $row->zhuanyuan;?>" href="<?php echo $site_url;?>wap/questionnaire/show/<?php echo $row->id;?>">
    <h3><img src="<?php echo (@$row->int) ? ((substr(@$row->int,0,4)=='http' || substr(@$row->int,0,1)=='/') ? @$row->int : $base_url.@$row->int)  : "";?>"/></h3>
    <h4><?php echo $row->zhuanyuan;?></h4>
