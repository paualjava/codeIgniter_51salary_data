...
class DbManage {
     ...
     */
    function __construct($host = 'localhost', $username = 'root', $password = 'mima', $database = 'codeIgniter_system2', $charset = 'utf8') {
        $this->host = $host;
...
@ob_end_flush();
     ...
     */
    function backup($tablename = '',$dir='', $pre="site_",$size='') {
        $dir = $dir ? $dir : './backup/';
        // 创建目录
        if (! is_dir ( $dir )) {
        ...
        if (count($tablename)>0) {
            ...
            {
                $tablename_single=$pre.$tablename_single;
                if(@mysql_num_rows(mysql_query("SHOW TABLES LIKE '".$tablename_single."'")) == 1) {
                } else {
                    $this->_showMsg('表-<b>' . $tablename_single .'</b>-不存在，请检查！',true);
                    die();
                }
                $this->_showMsg('正在备份表 <span class="imp">' . $tablename_single.'</span>');
                // 插入dump信息
                $sql = $this->_retrieve ();
                $sql2.= $this->_retrieve ();
                // 插入表结构信息
                $sql .= $this->_insert_table_structure ( $tablename_single );
                $sql2 .= $this->_insert_table_structure ( $tablename_single );
                // 插入数据
                $data = mysql_query ( "select * from " . $tablename_single );
                // 文件名前面部分
               // $filename = date ( 'YmdHis' ) . "_" . $tablename_single;
                $filename = $tablename_single;
                // 字段数量
                ...
                while ( $record = mysql_fetch_array ( $data ) ) {
                    ...
                    $sql2 .= $this->_insert_record ( $tablename_single, $num_fields, $record );
                    // 如果大于分卷大小，则写入文件
                    if (strlen ( $sql ) >= $size * 1024) {
                        $file = $filename.".sql";
                        if ($this->_write_file ( $sql, $file, $dir )) {
                            $this->_showMsg("表-<b>" . $tablename_single . "</b>-卷-<b>" . $p . "</b>-数据备份完成,备份文件 [ <span class='imp'>" .$dir . $file ."</span> ]");
                        } else {
                            $this->_showMsg("备份表 -<b>" . $tablename_single . "</b>- 失败",true);
                            return false;
                        }
                        // 下一个分卷
                        $p ++;
                ...
                if ($sql != "") {
                    ...
                    if ($this->_write_file ( $sql, $filename, $dir )) {
                        $this->_showMsg( "表-<b>" . $tablename_single . "</b>-卷-<b>" . $p . "</b>-数据备份完成,备份文件 [ <span class='imp'>" .$dir . $filename ."</span> ]");
                    } else {
            ...
            $this->_write_file ( $sql2, "all.sql", $dir );
        } else {
            $this->_showMsg('正在备份');
            // 备份全部表
