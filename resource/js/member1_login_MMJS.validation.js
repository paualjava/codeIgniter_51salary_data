...
验证最小字符数 jsv_minlength_[整数]
验证最大字符数 jsv_maxlength_[整数]

验证符合preg正则表达式 jsv_preg (preg正则表达式需写在元素的 data-jsv_preg 参数中)
...
jQuery.fn.MMJS_validation = function(){
	return this.each(function(){
		var $form = jQuery(this).data('check_stage',1);
		var fe_list = [];
		...
		$form.find('input,textarea,select').each(function(){
			...
			for ( var i=0,l=class_name_da.length;i<l;i++ ){
				...
				var m =/jsv_maxlength_([\d]+)/.exec(class_name_da[i]) 
				if ( m && m.length ){
					validation_list.push( {'rule':'maxlength','meta':m[1]} );
				}
				if ( class_name_da[i]=='jsv_preg' ){
					validation_list.push( {'rule':'preg','meta':$fe.data('jsv_preg')} );
				}
			}
			...
			$fe.focusin(function(){
				//jQuery(this).data('jsv_inited',true).unbind('focusin',arguments.callee);
			});
		...
		$form.MMJS_validate(null,true);

	});
}

jQuery.fn.MMJS_validate = function( can_be_lazy,hold_stage ){
	var validated = true;
	var $form = jQuery(this);
	var $fe = $form.find('input,textarea,select');
	
	var check_stage = $form.data('check_stage');
	if ( check_stage == 1 && !hold_stage ){
		$form.data('check_stage',2);
		check_stage = 2;
	}
	...
	$fe.each(function(){
		...
		var fe_event_registed = $fe_item.attr('mmjs_validation_fe_event_registed');
		
		if ( !fe_event_registed ){

			$fe_item.focusout(function(){
				
				window.setTimeout(function(){
					$form.MMJS_validate( can_be_lazy,true );
				},200);
			}).change(function(){
				window.setTimeout(function(){
					$form.MMJS_validate( can_be_lazy,true );
			...
			$fe_item.attr('mmjs_validation_fe_event_registed',true);
		}
				...
				if ( !marked ){
					...
					switch( rule_method ){
						...
						case 'cellphone':
							...
							break;
						case 'phone':
							var phone_regexp = /^(([0\+]\d{2,3}-)?(0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/;
						...
						case 'minlength':
							...
							break;
						case 'maxlength':
							var max_length = parseInt(rule_meta);
